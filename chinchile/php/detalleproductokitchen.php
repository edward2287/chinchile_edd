...
if ( $id == "" || $token == "") {
    ...
    if ($token== $token_tmp) {
        $sql = $con->prepare("SELECT count(id) FROM productos WHERE id=? AND activo=1 AND categoria = 'kitchen'  ");
        $sql->execute([$id]);
        if ($sql->fetchColumn() > 0) {
            $sql = $con->prepare("SELECT nombre, descripcion, precio, descuento FROM productos WHERE id=? AND activo=1 AND categoria = 'kitchen'  ");
            $sql->execute([$id]);
